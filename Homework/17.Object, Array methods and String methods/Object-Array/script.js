...
function secretName(anyNames) {
  return anyNames.map((item) => item[0]).join("");
}
...
function dividePrice(anyArr) {
    return anyArr.filter((item) => item.price = item.price/2)
}

console.log(dividePrice(arr4));
...
function reverseNum(anyNumbers) {
    return `${String(anyNumbers)}${String(anyNumbers).split('').reverse().join('')}`
}
